var SCanvasView = Backbone.View.extend({

    initialize: function () {

        console.log("initialize scanvas view");
        // body...
        var g;  
      var canvas;
      ...
      var self = this;
      canvas = document.getElementById('canvasTun');     
...
//        canvas.height = height;
      ...
      this.fillStrokeCircle = function(x,y,radius){
        if(radius<=0)
          radius = 0;
        g.beginPath();          
      ...
      this.dist = function(ax, ay, bx, by) {
        var dx = bx - ax;
        dy = by - ay;
     ...
     this. Marsaglia.createRandomized = function() {
        var now = new Date();
        return new Marsaglia((now / 60000) & 0xFFFFFFFF, now & 0xFFFFFFFF);
      };    
    
      // Noise functions and helpers
      this.PerlinNoise = function(seed) {
      ...
      this.Segment = function(x1,y1,x2,y2,thickness){
        this.x1 = x1;
        this.y1 = y1;
        this.x2 = x2;
        this.y2 = y2;
    ...
    drawInternal: function (){        
        ...
        var self = this;
        this.draw();
        setTimeout(this.drawInternal, self.drawRate);     
...
//            g.stroke();
        ...
        for(var w=0; w<self.worms.length; w++){
          var worm = worms[w];
          if(worm.ooo){
            worms.splice(w,1);
            w--;
        ...
        if(self.frameCount %2 ==0 && self.mousePressed && self.worms.length < 50){
          var direction = Math.atan2(self.pmouseY-self.mouseY, self.pmouseX-self.mouseX) + PI;
          var newWorm = new Worm(self.mouseX,self.mouseY);
          self.worms.push( newWorm );
